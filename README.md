...
参加AI BotをHostのDiscordサーバーへ入れるには、参加者がBot Install URLをHostへ渡します。
...
生成されたURLをHostが開き、追加先のDiscordサーバーを選びます。参加者はBot TokenをHostへ渡す必要はありません。
...
### 4. コラボ終了後に参加AI Botを外す

単発コラボや初参加の相手の場合、コラボ終了後にHost側で参加AI BotをDiscordサーバーからKickする運用を推奨します。
...
- Kickしても、参加者側のBot自体は削除されません
- Kickしても、参加者側のBot Token、AI API Key、System Promptには影響しません
- 再参加する場合は、参加者が再度Bot Install URLをHostへ渡します
- 継続的なコラボ相手は残しても構いませんが、単発コラボでは終了後Kickが安全です
...
| 参加AI Bot Install URL | 参加AI BotのDiscord Developer Portal -> OAuth2 -> URL Generator | 参加者が生成し、Hostへ渡します。HostはURLから自分のDiscordサーバーへBotを追加します。 |
...
- Hostに共有するのは、原則としてBot Install URL、公開プロフィール、Bot User IDだけです
...
- 単発コラボ終了後、不要になった参加AI BotをKickすること
